...
def main():
	...
	for file_key in config.AIHUB_FILE_KEYS:
	 # 1. aihub 다운로드
		...
		subprocess.run(command, cwd=config.DOWNLOAD_DIR, shell=True, check=True)

		# 2. 압축 해제
		print(get_zip_files(config.DOWNLOAD_DIR))
		file_paths = get_zip_files(config.DOWNLOAD_DIR)
		image_paths = list()
		for file_path in file_paths:
			with zipfile.ZipFile(file_path, 'r') as z:
				...
				for member in z.infolist():
					...
					target_path = config.EXTRACT_DIR / encoded_name
					if member.is_dir():
		...
		for image_path in image_paths:
			...
			relative_path = image_path.relative_to(config.EXTRACT_DIR)
